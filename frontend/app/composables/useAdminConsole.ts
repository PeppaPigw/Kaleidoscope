...
interface OpenApiOperation {
  ...
  requestBody?: {
    ...
    content?: Record<
      string,
      { schema?: Record<string, unknown>; example?: unknown }
    >;
  };
...
function inferProbeMode(
  endpoint: Pick<
    AdminEndpoint,
    "method" | "path" | "isMutation" | "isStream" | "requestBody"
  >,
): AdminProbeMode {
...
export function normalizeOpenApiCatalog(
    ...
    for (const methodKey of ROUTE_METHODS) {
      ...
      const summary =
        ...
        (operation.operationId
          ? humanizeIdentifier(operation.operationId)
          : `${method} ${path}`);
      ...
      const endpoint: AdminEndpoint = {
        ...
        operationId:
          operation.operationId ??
          `${methodKey}_${path.replace(/[/{}]/g, "_")}`,
        parameters: (operation.parameters ?? [])
          .map(normalizeParameter)
          .filter(
            (parameter): parameter is AdminEndpointParameter =>
              parameter !== null,
          ),
...
export function createRunnerSeed(
  ...
  const queryParams = Object.fromEntries(
    endpoint.parameters
      .filter(
        (parameter) =>
          parameter.location === "query" && parameter.defaultValue !== null,
      )
      .map((parameter) => [parameter.name, parameter.defaultValue]),
  );

  const bodyExample =
    endpoint.requestBody?.example ?? (endpoint.requestBody ? {} : null);
...
export function createHistoryRestoreSeed(
  entry: Pick<AdminRequestHistoryEntry, "pathParams" | "query" | "body">,
) {
  return {
...
export function sliceAutoProbeBatch<T>(
  ...
  const size = mode === "all" ? remaining : Math.min(batchSize, remaining);
...
function summarizeServiceMatrix(
  services: Record<string, string> | undefined,
): Pick<AdminProbeCard, "status" | "value" | "detail"> {
...
function statusFromCoverage(
  fields: Array<{ pct: number }> | undefined,
): AdminProbeStatus {
  ...
  const avgCoverage =
    fields.reduce((sum, field) => sum + field.pct, 0) / fields.length;
...
export function useAdminConsole() {
  ...
  async function loadCatalog() {
    ...
    if (!result.ok) {
      catalogError.value =
        result.errorMessage ?? `Failed to load OpenAPI (${result.status})`;
      endpoints.value = [];
    ...
    selectedEndpointId.value =
      selectedEndpointId.value &&
      endpoints.value.some(
        (endpoint) => endpoint.id === selectedEndpointId.value,
      )
        ? selectedEndpointId.value
        : (endpoints.value.find((endpoint) => endpoint.id === "GET /health")
            ?.id ??
  ...
  async function loadSamples() {
    ...
    samples.value = {
      paperId: paperResult.ok
        ? ((paperResult.data as { items?: Array<{ id?: string }> }).items?.[0]
            ?.id ?? null)
        : null,
      ...
      authorId: authorResult.ok
        ? firstIdFromCollection(
            (authorResult.data as { authors?: unknown[] } | undefined)?.authors,
            ["id", "author_id"],
          )
        : null,
      topicId: topicResult.ok
        ? firstIdFromCollection(
            (topicResult.data as { topics?: unknown[] } | undefined)?.topics,
          )
        : null,
      tagId: tagResult.ok ? firstIdFromCollection(tagResult.data) : null,
      experimentId: experimentResult.ok
        ? firstIdFromCollection(experimentResult.data)
        : null,
      feedId: feedResult.ok
        ? firstIdFromCollection(
            (feedResult.data as { items?: unknown[] } | undefined)?.items,
          )
        : null,
      alertRuleId: alertRuleResult.ok
        ? firstIdFromCollection(
            (alertRuleResult.data as { rules?: unknown[] } | undefined)?.rules,
          )
        : null,
  ...
  function findEndpoint(endpointId: string | null) {
    ...
    return (
      endpoints.value.find((endpoint) => endpoint.id === endpointId) ?? null
    );
  }
  ...
  function initializeRouteProbes() {
    ...
    const candidates = endpoints.value.filter((endpoint) =>
      canAutoProbe(endpoint),
    );
  ...
  async function loadRouteProbes(mode: "next" | "all" = "next") {
    ...
    const batch = sliceAutoProbeBatch(
      routeProbeReadyIds.value,
      routeProbeCursor.value,
      mode,
    );
    ...
    try {
      for (
        ...
        index += AUTO_PROBE_CONCURRENCY
      ) {
        const endpointIds = batch.items.slice(
          index,
          index + AUTO_PROBE_CONCURRENCY,
        );
        ...
        await Promise.all(
          endpointIds.map(async (endpointId) => {
            ...
            if (
              !endpoint?.autoProbePath ||
              generation !== routeProbeGeneration.value
            ) {
              return;
            }
  ...
  async function loadProbes() {
    ...
    const serviceSummary = summarizeServiceMatrix(
      (servicesResult.data as { services?: Record<string, string> } | undefined)
        ?.services,
    );
    ...
    const coverage = coverageResult.data as
      ...
      | undefined;
    const authMe = authResult.data as
      | { mode?: string; user_id?: string }
      | undefined;
    ...
    probes.value = [
      {
        ...
        status:
          healthResult.ok &&
          (healthResult.data as { status?: string })?.status === "ok"
            ? "ok"
            : "error",
        value: healthResult.ok
          ? String((healthResult.data as { version?: string })?.version ?? "up")
          : "down",
      ...
      },
      {
        ...
        status: coverageResult.ok
          ? statusFromCoverage(coverage?.fields)
          : "error",
        ...
        note:
          coverageResult.ok && coverage?.fields
            ? coverage.fields
                .slice(0, 3)
                .map((field) => `${field.field} ${field.pct}%`)
                .join(" · ")
            : undefined,
      },
    ];
  ...
  const summary = computed(() => {
    ...
    const autoCandidates = endpoints.value.filter(
      (endpoint) => endpoint.probeMode === "safe",
    ).length;
    const autoVerified = endpoints.value.filter(
      (endpoint) => endpoint.autoProbeStatus === "ok",
    ).length;
    const autoFailing = endpoints.value.filter(
      (endpoint) => endpoint.autoProbeStatus === "error",
    ).length;
    const blockedAuto = endpoints.value.filter(
      (endpoint) =>
        endpoint.probeMode === "safe" &&
        endpoint.autoProbeStatus === "warning" &&
        !endpoint.autoProbePath,
    ).length;
    return {
      ...
      autoFailing,
      blockedAuto,
      manualOnly: endpoints.value.filter(
        (endpoint) => endpoint.probeMode !== "safe",
      ).length,
    };
  });
  ...
  const groupedEndpoints = computed(() => {
    return filteredEndpoints.value.reduce<Record<string, AdminEndpoint[]>>(
      (groups, endpoint) => {
  ...
  const crossEndpointHistory = computed(() => {
    return requestHistory.value
      .filter(
        (entry) =>
          entry.endpointId && entry.endpointId !== selectedEndpointId.value,
      )
      .slice(0, 6);
  });
  ...
  const autoProbeQueue = computed<AdminAutoProbeQueue>(() => {
    const readyCount = endpoints.value.filter(
      (endpoint) =>
        endpoint.probeMode === "safe" && Boolean(endpoint.autoProbePath),
    ).length;
    const blockedCount = endpoints.value.filter(
      (endpoint) => endpoint.probeMode === "safe" && !endpoint.autoProbePath,
    ).length;
    const succeededCount = endpoints.value.filter(
      (endpoint) => endpoint.autoProbeStatus === "ok",
    ).length;
    const failedCount = endpoints.value.filter(
      (endpoint) => endpoint.autoProbeStatus === "error",
    ).length;
    ...
    return {
      ...
      progressPct:
        readyCount === 0
          ? 100
          : Math.round((processedCount / readyCount) * 100),
  ...
  async function runQuickAction(
    ...
    historyContext: Partial<AdminRunnerPayload> & {
      endpointId?: string | null;
    } = {},
  ) {
