...
describe('resolveAutoProbePath', () => {
  it('resolves path parameters with sampled ids for automatic probing', () => {
    ...
    const contentEndpoint = endpoints.find(endpoint => endpoint.path.includes('/content'))
    const timeseriesEndpoint = endpoints.find(endpoint => endpoint.path.includes('/timeseries'))
...
describe('sliceAutoProbeBatch', () => {
  it('returns the next probe page without overshooting', () => {
    expect(
      sliceAutoProbeBatch(['a', 'b', 'c', 'd'], 1, 'next', 2),
    ).toEqual({
      items: ['b', 'c'],
      nextCursor: 3,
    })
    ...
    expect(
      sliceAutoProbeBatch(['a', 'b', 'c', 'd'], 3, 'next', 2),
    ).toEqual({
      items: ['d'],
      nextCursor: 4,
    })
  ...
  it('can drain all remaining probes in a single pass', () => {
    expect(
      sliceAutoProbeBatch(['a', 'b', 'c', 'd'], 1, 'all', 2),
    ).toEqual({
      items: ['b', 'c', 'd'],
      nextCursor: 4,
    })
