...
const resultsFocusedPaper = computed<OpenAlexPaper | null>(
  () =>
    searchResults.value.find((p) => p.openalex_id === resultsFocusedId.value) ??
    null,
);
...
async function buildGraph() {
  ...
  try {
    ...
    if (res.isolated_count > 0) {
      console.warn(
        `${res.isolated_count} paper(s) were excluded from the graph due to no citation connections`,
      );
    }
    ...
    buildError.value =
      err instanceof Error ? err.message : "Graph build failed";
  } finally {
...
const graphSelectedNode = computed<OpenAlexGraphNode | null>(
  () =>
    graphNodes.value.find((n) => n.openalex_id === graphSelectedId.value) ??
    null,
);
...
const detailConnections = computed(() => {
  const paper = detailPaper.value;
  if (!paper) return 0;
  return graphEdges.value.filter(
    (edge) =>
      edge.source === paper.openalex_id || edge.target === paper.openalex_id,
  ).length;
});
    ...
    <Transition name="kss-fade">
      <div v-if="phase === 'spotlight'" class="kss-spotlight">
        <div class="kss-spotlight__inner">
          ...
          <div
            class="kss-spotlight__box"
            :class="{ 'kss-spotlight__box--loading': isSearching }"
          >
            <svg
              class="kss-spotlight__icon"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="1.8"
          ...
          <p v-if="searchError" class="kss-spotlight__error">
            {{ searchError }}
          </p>
      ...
      <div v-if="phase !== 'spotlight'" class="kss-layout">
        ...
        <nav class="kss-nav">
          <div v-if="phase === 'graph'" class="kss-nav__modes">
            <button
              ...
              :class="[
                'kss-nav__mode',
                layoutMode === m && 'kss-nav__mode--active',
              ]"
              @click="layoutMode = m"
            >
              {{ m }}
        ...
        <div class="kss-body">
            ...
            <div class="kss-left__search">
              <div class="kss-search-field">
                <svg
                  ...
                  stroke="currentColor"
                  stroke-width="2"
                  style="flex-shrink: 0; color: #6e7979"
                >
                  <circle cx="11" cy="11" r="8" />
                  <path d="m21 21-4.35-4.35" />
                </svg>
            ...
            <template v-if="phase === 'results'">
              <div class="kss-left__header">
                ...
                <div class="kss-left__header-actions">
                  ...
                  <button class="kss-link-btn" @click="clearSelection">
                    None
                  </button>
                </div>
              </div>
              ...
              <ul class="kss-list">
                <li
                  ...
                  :class="[
                    'kss-card',
                    selectedIds.has(p.openalex_id) && 'kss-card--checked',
                    resultsFocusedId === p.openalex_id && 'kss-card--focused',
                  ]"
                ...
                >
                  ...
                  >
                    <div
                      :class="[
                        'kss-checkbox',
                        selectedIds.has(p.openalex_id) && 'kss-checkbox--on',
                      ]"
                    >
                      <svg
                        v-if="selectedIds.has(p.openalex_id)"
                        width="10"
                        height="10"
                        viewBox="0 0 10 10"
                        fill="none"
              ...
              <div v-if="selectedIds.size > 0" class="kss-build-footer">
                ...
                <p v-if="buildError" class="kss-build-error">
                  {{ buildError }}
                </p>
              </div>
            </template>
            ...
            <template v-else-if="phase === 'graph'">
              ...
              <div class="kss-origin-section">
                <div class="kss-origin-section__header">
                  <span class="kss-label"
                    >Selected ·
                    {{ graphNodes.filter((n) => n.is_origin).length }}</span
                  >
                </div>
                <ul class="kss-origin-list">
                  <li
                    ...
                    :class="[
                      'kss-origin-item',
                      graphSelectedId === n.openalex_id &&
                        'kss-origin-item--selected',
                    ]"
              ...
              <ul v-if="layoutMode === 'list'" class="kss-list">
                <li
                  ...
                  :class="[
                    'kss-card-list',
                    graphSelectedId === n.openalex_id &&
                      'kss-card-list--selected',
                  ]"
                  @click="handleGraphSelect(n.openalex_id)"
                >
                  <div class="kss-card-list__header">
                    <span class="kss-card-list__author">{{
                      firstAuthorFullName(n)
                    }}</span>
              ...
              <ul v-else class="kss-list">
                <li
                  ...
                  :key="n.openalex_id"
                  :class="[
                    'kss-card kss-card--node',
                    graphSelectedId === n.openalex_id && 'kss-card--selected',
                  ]"
                  @click="handleGraphSelect(n.openalex_id)"
                >
          ...
          >
            ...
            <p class="kss-center__footer">
              Click node to explore · Click background to deselect · Drag to
              reposition
            </p>
          </section>
          ...
          <section
            ...
            class="kss-center kss-center--empty"
          >
          ...
          <div
            v-if="
              (phase === 'graph' && layoutMode !== 'list') ||
              (phase === 'results' && detailPaper)
            "
            class="kss-handle"
          ...
          <aside
            ...
            v-show="
              phase === 'results' || layoutMode !== 'map' || graphSelectedNode
            "
            class="kss-right"
            :style="rightStyle"
          >
            ...
            <div v-else class="kss-detail">
              ...
              <div class="kss-detail__top">
                ...
                <div
                  v-if="
                    phase === 'graph' && detailPaper.is_origin !== undefined
                  "
                  class="kss-detail__eyebrow"
                >
                  <span class="kss-detail__rule" />
                  <span
                    :class="[
                      'kss-detail__tag',
                      detailPaper.is_origin
                        ? 'kss-detail__tag--origin'
                        : 'kss-detail__tag--ref',
                    ]"
                  >
                ...
                >
                  ...
                  <span
                    :class="[
                      'kss-detail__tag',
                      selectedIds.has(detailPaper.openalex_id)
                        ? 'kss-detail__tag--origin'
                        : 'kss-detail__tag--ref',
                    ]"
                  >
                ...
                <div class="kss-detail__authors">
                  ...
                  <div class="kss-detail__author-list">
                    ...
                    <template v-else>
                      <span
                        v-for="name in detailPaper.authors"
                        :key="name"
                        class="kss-detail__author-plain"
                        >{{ name }}</span
                      >
                    </template>
                  </div>
                ...
                <div v-if="detailPaper.venue" class="kss-detail__meta-item">
                  ...
                  <span
                    class="kss-detail__meta-val kss-detail__meta-val--italic"
                    >{{ venueDisplay(detailPaper) }}</span
                  >
                </div>
                ...
                <div class="kss-detail__stats">
                  ...
                  <div class="kss-detail__stat kss-detail__stat--border">
                    ...
                    <p
                      class="kss-detail__stat-num kss-detail__stat-num--accent"
                    >
                      {{ formatCitations(detailPaper.cited_by_count) }}
                    </p>
                  </div>
                  <div class="kss-detail__stat">
                    <p class="kss-label kss-label--xs">
                      {{ phase === "graph" ? "Connections" : "Similarity" }}
                    </p>
                    <p
                      class="kss-detail__stat-num kss-detail__stat-num--primary"
                    >
                      ...
                      <template v-else>
                        {{
                          detailPaper.similarity_score != null
                            ? (detailPaper.similarity_score * 100).toFixed(0) +
                              "%"
                            : "—"
                        }}
                      </template>
              ...
              <div class="kss-detail__bottom">
                <!-- Abstract -->
                <section
                  v-if="detailPaper.abstract"
                  class="kss-detail__section"
                >
                  <div class="kss-detail__section-head">
                    ...
                    >
                      ...
                      >
                        <path
                          d="M5 8l6 6M4 6h7M2 4h7m6 16-3-8-3 8m0 0h6M21 4c-2 5-5 8.5-9 11"
                        />
                      </svg>
                      {{
                        showTranslated
                          ? "Hide Translation"
                          : isPending(detailPaper.abstract)
                            ? "Translating…"
                            : "中文"
                      }}
                    </button>
                  ...
                  <!-- Chinese Translation -->
                  <div
                    v-if="showTranslated && translatedAbstract"
                    class="kss-detail__abstract"
                    style="margin-top: 16px"
                  >
                    <div
                      class="kss-detail__abstract-bar"
                      style="background: #00595c"
                    />
                    <div class="kss-detail__abstract-content">
                      <span
                        class="kss-label kss-label--xs"
                ...
                <div class="kss-detail__tag-groups">
                  <div
                    v-if="detailPaper.topics?.length"
                    class="kss-detail__tag-group"
                  >
                    <span class="kss-label kss-label--xs">Topics</span>
                    <div class="kss-detail__tag-list">
                      <span
                        v-for="t in detailPaper.topics.slice(0, 6)"
                        :key="t.id"
                        class="kss-tag kss-tag--topic"
                        >{{ t.display_name }}</span
                      >
                    </div>
                  </div>
                  <div
                    v-if="detailPaper.keywords?.length"
                    class="kss-detail__tag-group"
                  >
                    <span class="kss-label kss-label--xs">Keywords</span>
                    <div class="kss-detail__tag-list">
                      <span
                        v-for="k in detailPaper.keywords.slice(0, 8)"
                        :key="k.display_name"
                        class="kss-tag kss-tag--keyword"
                        >{{ k.display_name }}</span
                      >
                    </div>
                  </div>
                  <div
                    v-if="detailPaper.concepts?.length"
                    class="kss-detail__tag-group"
                  >
                    <span class="kss-label kss-label--xs">Concepts</span>
                    <div class="kss-detail__tag-list">
                      <span
                        v-for="c in detailPaper.concepts.slice(0, 6)"
                        :key="c.id"
                        class="kss-tag kss-tag--concept"
                        >{{ c.display_name }}</span
                      >
                    </div>
                  </div>
                ...
                <div class="kss-detail__actions">
                  ...
                  <div v-if="phase === 'graph'" class="kss-collection-action">
                    ...
                    <div
                      ...
                      class="kss-collection-dropdown"
                    >
                      <div
                        v-if="collections.length === 0"
                        class="kss-collection-dropdown__empty"
                      >
                    ...
                    <div
                      v-if="addToCollectionSuccess"
                      class="kss-collection-message kss-collection-message--success"
                    >
                      {{ addToCollectionSuccess }}
                    </div>
                    <div
                      v-if="addToCollectionError"
                      class="kss-collection-message kss-collection-message--error"
                    >
                  ...
                  <a
                    :href="
                      detailPaper.openalex_url ??
                      `https://openalex.org/${detailPaper.openalex_id}`
                    "
                    target="_blank"
                    rel="noopener"
...
.kss-spotlight__box:focus-within {
  border-color: #00595c;
  box-shadow:
    0 4px 24px rgba(0, 89, 92, 0.12),
    0 0 0 3px rgba(0, 89, 92, 0.08);
}
.kss-spotlight__box--loading {
  opacity: 0.7;
}
...
.kss-spotlight__input::placeholder {
  color: #9eacac;
}
.kss-spotlight__input:disabled {
  opacity: 0.5;
}
...
.kss-list::-webkit-scrollbar {
  width: 4px;
}
.kss-list::-webkit-scrollbar-thumb {
  background: rgba(110, 121, 121, 0.2);
...
.kss-card--checked {
  background: #f0fafa;
  border-left: 3px solid #00595c;
}
.kss-card--focused {
  background: #f4fafa;
  outline: 1px solid #bde0e0;
}
.kss-card--checked.kss-card--focused {
  background: #e8f5f5;
}
.kss-card--selected {
  background: #e8f4f4;
...
.kss-card--selected .kss-card__title {
  color: #00595c;
}
.kss-card__author {
  font: 400 0.5625rem/1 var(--font-sans, sans-serif);
...
.kss-build-btn:hover:not(:disabled) {
  background: #0d7377;
}
.kss-build-btn:disabled {
  opacity: 0.5;
  cursor: default;
}
.kss-build-error {
...
.kss-origin-list::-webkit-scrollbar {
  width: 3px;
}
.kss-origin-list::-webkit-scrollbar-thumb {
  background: rgba(0, 89, 92, 0.2);
  border-radius: 9999px;
}
.kss-origin-item {
  display: flex;
...
.kss-origin-item:hover {
  background: rgba(0, 89, 92, 0.06);
}
.kss-origin-item--selected {
...
.kss-card-list--selected .kss-card-list__title {
  color: #00595c;
}
...
.kss-right::-webkit-scrollbar {
  width: 4px;
}
.kss-right::-webkit-scrollbar-thumb {
  background: rgba(110, 121, 121, 0.18);
  border-radius: 9999px;
}
...
.kss-detail__meta-item {
  display: flex;
  flex-direction: column;
  gap: 3px;
}
.kss-detail__meta-val {
  font: 400 0.8125rem/1.3 var(--font-serif, serif);
  color: #1a1c1b;
}
.kss-detail__meta-val--italic {
  font-style: italic;
}
...
.kss-detail__stat--border {
  border-left: 1px solid #e2e3e0;
  border-right: 1px solid #e2e3e0;
}
.kss-detail__stat-num {
  font: 700 1.1rem/1 var(--font-mono, monospace);
  color: #3e4949;
  margin-top: 4px;
}
.kss-detail__stat-num--primary {
  color: #00595c;
}
.kss-detail__stat-num--accent {
  color: #7f6421;
}
...
.kss-translate-btn:hover:not(:disabled) {
  color: #0d7377;
  border-color: #0d7377;
}
.kss-translate-btn:disabled {
  opacity: 0.5;
  cursor: default;
}
.kss-detail__abstract {
  position: relative;
  padding-left: 12px;
}
...
.kss-tag--keyword {
  background: rgba(127, 100, 33, 0.1);
  color: #7f6421;
}
.kss-tag--concept {
  background: rgba(94, 94, 94, 0.09);
  color: #3e4949;
}
...
.kss-detail__action-link {
  ...
  justify-content: space-between;
  padding: 9px 0;
  border-bottom: 1px solid #7f6421;
  font: 600 0.625rem/1 var(--font-sans, sans-serif);
  letter-spacing: 0.14em;
  text-transform: uppercase;
  color: #7f6421;
...
.kss-detail__action-link--muted {
  border-color: #bec9c9;
  color: #6e7979;
}
.kss-detail__action-link:hover {
  opacity: 0.65;
}
.kss-detail__action-link:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
...
/* ══ TRANSITIONS ═══════════════════════════════════════════ */
.kss-fade-enter-active,
.kss-fade-leave-active {
...
.kss-slide-enter-active {
  transition:
    opacity 0.3s 0.1s,
    transform 0.3s 0.1s;
}
.kss-slide-enter-from {
  opacity: 0;
  transform: translateY(12px);
}
.kss-slide-leave-active {
