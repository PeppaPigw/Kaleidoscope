      ...
      <div v-if="sources.length" class="qa-sources-card">
        ...
        <div class="qa-sources-list">
          <div v-for="(src, i) in sources" :key="i" class="qa-source-chip">
            ...
            <div class="qa-source-content">{{ truncate(src.content, 180) }}</div>
            <div v-if="src.score" class="qa-source-score-bar">
              <div class="qa-score-fill" :style="{ width: `${Math.min(src.score * 100, 100)}%` }" />
            </div>
          </div>
        </div>
      </div>
...
async function handleAsk() {
  ...
  try {
    ...
    if (result.error) {
      error.value = result.error === 'ragflow_unavailable'
...
.qa-brand { display: flex; align-items: center; gap: 8px; }
.qa-brand-icon {
...
.qa-brand-label {
  font: 600 0.9rem/1 var(--font-sans); color: var(--color-text, #e2e8f0);
...
.qa-badge.ready { background: rgba(16,185,129,0.12); color: #34d399; }
.qa-badge.ready .qa-badge-dot { background: #34d399; box-shadow: 0 0 6px #34d399; }
.qa-badge.syncing { background: rgba(251,191,36,0.12); color: #fbbf24; }
.qa-badge.syncing .qa-badge-dot { background: #fbbf24; animation: pulse-dot 1.2s infinite; }
...
.qa-input-wrap.focused {
  ...
  box-shadow: 0 0 0 3px rgba(99,102,241,0.1), 0 2px 10px rgba(0,0,0,0.15);
}
.qa-input-icon { color: rgba(148,163,184,0.5); flex-shrink: 0; }
.qa-input {
  flex: 1; border: none; background: transparent; outline: none;
  font: 400 0.85rem/1.4 var(--font-sans); color: var(--color-text, #e2e8f0);
  min-width: 0;
}
.qa-input::placeholder { color: rgba(148,163,184,0.4); }
.qa-submit {
  width: 32px; height: 32px; display: grid; place-items: center;
  border: none; border-radius: 8px; cursor: pointer;
  background: rgba(148,163,184,0.1); color: rgba(148,163,184,0.4);
  transition: all 0.2s ease; flex-shrink: 0;
}
.qa-submit.active { background: linear-gradient(135deg, #6366f1, #8b5cf6); color: #fff; }
.qa-submit:disabled { opacity: 0.3; cursor: not-allowed; }
.qa-btn-spinner {
  width: 14px; height: 14px; border: 2px solid rgba(255,255,255,0.3);
  border-top-color: #fff; border-radius: 50%; animation: spin 0.5s linear infinite;
}
@keyframes spin { to { transform: rotate(360deg); } }

/* Loading */
.qa-loading { display: flex; flex-direction: column; gap: 8px; }
.qa-pulse-bar {
  height: 3px; border-radius: 2px; background: rgba(99,102,241,0.1); overflow: hidden;
}
.qa-pulse-track {
...
.qa-loading-text {
  font: 400 0.75rem/1 var(--font-sans); color: rgba(148,163,184,0.6);
}

/* Answer card */
.qa-answer-card {
  background: rgba(30,41,59,0.45); border: 1px solid rgba(148,163,184,0.1);
  border-radius: 12px; overflow: hidden;
  backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
}
.qa-answer-header {
  display: flex; align-items: center; gap: 6px;
  padding: 10px 14px; border-bottom: 1px solid rgba(148,163,184,0.08);
  font: 500 0.7rem/1 var(--font-mono, monospace);
  color: rgba(148,163,184,0.6); text-transform: uppercase; letter-spacing: 0.05em;
}
.qa-grounding-pill {
...
/* Sources card */
.qa-sources-card {
  background: rgba(30,41,59,0.3); border: 1px solid rgba(148,163,184,0.08);
  border-radius: 12px; overflow: hidden;
}
.qa-sources-header {
  display: flex; align-items: center; gap: 6px;
  padding: 10px 14px; border-bottom: 1px solid rgba(148,163,184,0.06);
  font: 500 0.7rem/1 var(--font-mono); color: rgba(148,163,184,0.5);
  text-transform: uppercase; letter-spacing: 0.05em;
}
.qa-sources-list { display: flex; flex-direction: column; }
.qa-source-chip {
  display: grid; grid-template-columns: 28px 1fr; gap: 10px;
  padding: 10px 14px; border-bottom: 1px solid rgba(148,163,184,0.04);
  transition: background 0.15s ease;
}
.qa-source-chip:last-child { border-bottom: none; }
.qa-source-chip:hover { background: rgba(99,102,241,0.04); }
.qa-source-rank {
  font: 700 0.7rem/28px var(--font-mono);
  color: rgba(99,102,241,0.6); text-align: center;
}
.qa-source-content {
  font: 400 0.78rem/1.5 var(--font-serif);
  color: rgba(226,232,240,0.8); word-break: break-word;
}
.qa-source-score-bar {
  grid-column: 2; height: 3px; border-radius: 2px;
  background: rgba(99,102,241,0.1); margin-top: 4px;
}
.qa-score-fill {
...
.qa-error-card {
  ...
  color: #f87171; font: 400 0.8rem/1.4 var(--font-sans);
}
...
.qa-meta-item {
  font: 400 0.65rem/1 var(--font-mono); color: rgba(148,163,184,0.4);
}

/* Transitions */
.qa-slide-enter-active { animation: slide-in 0.3s ease; }
.qa-slide-leave-active { animation: slide-in 0.2s ease reverse; }
@keyframes slide-in {
