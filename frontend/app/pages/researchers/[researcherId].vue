...
const enrichResult = ref<{ status: string; changes?: string[]; match_reason?: string } | null>(null)
...
const hero = computed<ResearcherHeroProps>(() => {
  ...
  const affiliation = p.affiliations?.[0]?.name || p.affiliation || p.raw_affiliation || ''
...
const papers = computed<PaperEntry[]>(() =>
  (profile.value?.papers ?? []).map(p => ({
    ...
    author_position: p.author_position != null ? Number(p.author_position) : null,
...
const hasS2Stats = computed(() => !!(profile.value?.citation_count || profile.value?.paper_count))
...
async function enrichFromS2() {
  ...
  try {
    const result = await apiFetch<{ status: string; changes?: string[]; match_reason?: string }>(
      `/researchers/${researcherId.value}/enrich`,
      { method: 'POST' },
    )
...
function handlePaperClick(paper: SignaturePaper | PaperEntry) {
  ...
  const navId = ('library_paper_id' in paper && paper.library_paper_id) ? paper.library_paper_id : paper.id
...
<template>
  <div class="ks-researcher-profile">
    <KsPageHeader title="Researcher Profile" :subtitle="profile?.display_name ?? `ID ${researcherId}`" />
    ...
    <div class="ks-researcher-profile__content">
      ...
      <div class="ks-enrich-bar">
        <div class="ks-enrich-bar__meta">
          <span v-if="profile?.enriched_at" class="ks-enrich-bar__status ks-enrich-bar__status--ok">
            ✓ Synced from Semantic Scholar · {{ new Date(profile.enriched_at).toLocaleDateString() }}
          </span>
          <span v-else class="ks-enrich-bar__status">Profile not yet enriched from Semantic Scholar</span>
        ...
        <div class="ks-enrich-bar__actions">
          <span v-if="enrichResult?.status === 'no_match'" class="ks-enrich-bar__feedback ks-enrich-bar__feedback--warn">
            No match found on Semantic Scholar
          </span>
          <span v-else-if="enrichResult?.status === 'ok'" class="ks-enrich-bar__feedback ks-enrich-bar__feedback--ok">
            ✓ Updated: {{ enrichResult.changes?.join(', ') }}
          </span>
          <span v-else-if="enrichResult?.status === 'error'" class="ks-enrich-bar__feedback ks-enrich-bar__feedback--warn">
            Enrichment failed
          </span>

          <button class="ks-enrich-bar__btn" :disabled="enriching" @click="enrichFromS2">
      ...
      <div class="ks-stats-grid">
        ...
        <div class="ks-stats-grid__group">
          ...
          <div class="ks-stats-grid__row">
            <div class="ks-stats-grid__stat">
              <span class="ks-stats-grid__val">{{ profile?.paper_count_in_library ?? 0 }}</span>
              <span class="ks-stats-grid__key">papers</span>
            </div>
            <div class="ks-stats-grid__stat">
              <span class="ks-stats-grid__val">{{ (profile?.total_citations_in_library ?? 0).toLocaleString() }}</span>
        ...
        <div v-if="hasS2Stats" class="ks-stats-grid__group ks-stats-grid__group--s2">
          <span class="ks-stats-grid__label">Semantic Scholar global</span>
          <div class="ks-stats-grid__row">
            <div v-if="profile?.paper_count" class="ks-stats-grid__stat">
              <span class="ks-stats-grid__val">{{ profile.paper_count.toLocaleString() }}</span>
              <span class="ks-stats-grid__key">papers</span>
            </div>
            <div v-if="profile?.citation_count" class="ks-stats-grid__stat">
              <span class="ks-stats-grid__val">{{ profile.citation_count.toLocaleString() }}</span>
              <span class="ks-stats-grid__key">citations</span>
            </div>
        ...
        <div v-if="profile?.aliases?.length" class="ks-stats-grid__group">
          ...
          <div class="ks-stats-grid__aliases">
            <span v-for="alias in profile.aliases.slice(0, 5)" :key="alias" class="ks-stats-grid__alias">{{ alias }}</span>
          </div>
        </div>
      </div>
...
.ks-enrich-bar__status { font-size: 13px; color: var(--color-secondary, #666); }
.ks-enrich-bar__status--ok { color: var(--color-success, #2e7d32); }
...
.ks-enrich-bar__s2link:hover { text-decoration: underline; }

.ks-enrich-bar__actions { display: flex; align-items: center; gap: 12px; }

.ks-enrich-bar__feedback { font-size: 13px; }
.ks-enrich-bar__feedback--ok { color: var(--color-success, #2e7d32); }
.ks-enrich-bar__feedback--warn { color: var(--color-warning, #e65100); }
...
.ks-enrich-bar__btn:hover:not(:disabled) { background: var(--color-primary-light, #e8f0fe); }
.ks-enrich-bar__btn:disabled { opacity: 0.6; cursor: not-allowed; }
...
.ks-stats-grid__group:last-child { border-right: none; padding-right: 0; }
...
.ks-stats-grid__row { display: flex; gap: 20px; align-items: flex-end; }

.ks-stats-grid__stat { display: flex; flex-direction: column; align-items: flex-start; gap: 2px; }
...
.ks-stats-grid__group--s2 .ks-stats-grid__val { color: var(--color-accent-decorative, #c4a35a); }

.ks-stats-grid__aliases { display: flex; flex-wrap: wrap; gap: 6px; }
