...
function normalizeStreamAlert(payload: StreamPayload): StreamAlert | null {
  ...
  return {
    id: String(data.id ?? data.paper_id ?? `${payload.type}-${Date.now()}`),
    ...
    created_at: String(data.created_at ?? payload.timestamp ?? new Date().toISOString()),
  }
}
...
export function useAlertStream() {
  ...
  function connect() {
    ...
    eventSource.onmessage = (event) => {
      try {
        const alert = normalizeStreamAlert(JSON.parse(event.data) as StreamPayload)
        if (!alert) return
  ...
  return { lastAlert, isConnected, unreadCount, connect, disconnect, clearUnread }
}
