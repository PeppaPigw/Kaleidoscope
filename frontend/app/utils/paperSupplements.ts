import type {
  SupplementItem,
  SupplementItemType,
} from "../components/paper/supplements";
...
function getProvider(url: string): string | null {
  ...
  if (host === "huggingface.co" || host === "hf.co") return "Hugging Face";
  if (host === "zenodo.org" || url.includes("doi.org/10.5281/zenodo"))
    return "Zenodo";
  if (host === "figshare.com") return "Figshare";
  if (host === "kaggle.com") return "Kaggle";
  if (host === "modelscope.cn") return "ModelScope";
  return null;
}
...
function classifyCandidate(candidate: Candidate): {
  ...
  if (candidate.explicitType === "code") {
    return {
      type: "code",
      label: provider ? `${provider} Repository` : "Code Repository",
    };
  }
  ...
  if (candidate.explicitType === "dataset") {
    return {
      type: "dataset",
      label: provider ? `${provider} Dataset` : "Dataset Download",
    };
  }

  if (
    candidate.explicitType === "weights" ||
    candidate.explicitType === "model"
  ) {
    return {
      type: "weights",
      label: provider ? `${provider} Weights` : "Model Weights",
    };
  }
...
function collectMetadataCandidates(
  rawMetadata: PaperRawMetadata | null | undefined,
): Candidate[] {
...
function collectRemoteUrlCandidates(
  remoteUrls: RemoteUrl[] | null | undefined,
): Candidate[] {
  return (remoteUrls ?? [])
    .filter((record): record is RemoteUrl & { url: string } =>
      Boolean(record?.url),
    )
    .map((record) => ({
      url: record.url,
    ...
    if (line.includes("http://") || line.includes("https://")) {
      for (const match of line.matchAll(URL_PATTERN)) {
        ...
        candidates.push({
          ...
          snippet: [currentHeading, previousLine, line]
            .filter(Boolean)
            .join(" "),
          source: "markdown",
        });
