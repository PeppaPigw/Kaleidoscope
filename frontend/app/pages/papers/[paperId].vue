...
import type {
  MethodItem,
  ResultItem,
} from "~/components/paper/MethodsResultsSlice.vue";
  ...
  meta: [
    {
      name: "description",
      content:
        "Detailed paper analysis with claims, methods, results, and reproducibility.",
    },
  ],
});
...
const authors = computed<PaperAuthor[]>(() => {
  ...
  if (structuredAuthors.length > 0) return structuredAuthors;

  const rawAuthors =
    paper?.raw_metadata?.authors_parsed ?? paper?.raw_metadata?.authors ?? [];
...
const methods: MethodItem[] = [
  {
    id: "m1",
    name: "SciBERT-Decomposer",
    description:
      "Fine-tuned SciBERT for sentence-to-claim decomposition with NLI filtering.",
    type: "model",
  },
  {
    id: "m2",
    name: "BioASQ-Claims",
    description:
      "Extended BioASQ dataset with 12K manually annotated atomic claims.",
    type: "dataset",
  },
  {
    id: "m3",
    name: "Evidence F1",
    description: "F1 score measuring claim-evidence alignment accuracy.",
    type: "metric",
  },
  {
    id: "m4",
    name: "RAG-Paragraph",
    description: "Standard paragraph-level retrieval baseline.",
    type: "baseline",
  },
];
...
const reproAttempts: ReproductionAttempt[] = [
  {
    id: "ra1",
    team: "Allen AI (Semantic Scholar)",
    date: "2025-08",
    success: "full",
    notes:
      "Successfully reproduced main results within 1% margin on BioASQ benchmark.",
  },
  {
    id: "ra2",
    team: "UIUC NLP Lab",
    ...
    success: "partial",
    notes:
      "Reproduced claim extraction, but evidence alignment was 3% lower than reported.",
  },
];
...
watch(
  ...
  async (id) => {
    ...
    const [claimsResult, contentResult, similarResult, highlightsResult] =
...
async function loadPaperData() {
  ...
  try {
    paperData.value = await apiFetch<PaperDetailResponse>(
      `/papers/${paperId.value}`,
    );
  } catch {
    paperData.value = null;
  }
...
onMounted(() => {
  ...
  if (paperId.value) {
    ...
    void loadDeepAnalysis(paperId.value).then(() => {
      if (isZh.value && paperId.value) void loadDeepAnalysisZh(paperId.value);
    });
...
watch(paperId, (currentPaperId, previousPaperId) => {
  if (previousPaperId && currentPaperId !== previousPaperId) {
    ...
    void loadDeepAnalysis(currentPaperId).then(() => {
      if (isZh.value) void loadDeepAnalysisZh(currentPaperId);
    });
...
const deepAnalysisZhStatus = ref<
  "idle" | "loading" | "translating" | "ok" | "error"
>("idle");
...
async function loadDeepAnalysisZh(id: string) {
  ...
  try {
    ...
    deepAnalysisZhStatus.value =
      detail === "translating" ? "translating" : "error";
    deepAnalysisZhText.value = null;
  }
}

const deepAnalysisDisplay = computed(() =>
  isZh.value && deepAnalysisZhText.value
    ? deepAnalysisZhText.value
...
const availablePaperLinks = computed(() =>
  hasPaperLinks.value ? paperLinks.value : null,
);
    ...
    <div class="ks-paper-profile__layout">
      <div class="ks-paper-profile__main">
          ...
          <PaperFolio
            ...
            :year="
              paperData?.published_at
                ? new Date(paperData.published_at).getFullYear()
                : 0
            "
        ...
        <div class="ks-overview-image">
          ...
          <template v-else-if="deepAnalysisText">
            <div class="ks-overview-image__empty">
              ...
              <span class="ks-overview-image__hint"
                >根据 Deep Analysis 自动生成论文海报</span
              >
            </div>
          </template>
        </div>
        ...
        <div v-if="deepAnalysisText" id="ks-sec-analysis">
          <div
            v-if="isZh && deepAnalysisZhStatus === 'translating'"
            class="ks-zh-translating"
          >
            <span class="ks-overview-image__spinner" />
        ...
        <div id="ks-sec-related">
          <PaperRelatedConstellation
            :papers="relatedPapers"
            @paper-click="handleRelatedClick"
          />
        </div>
      </div>
      ...
      <aside class="ks-paper-profile__sidebar">
        ...
        <div v-if="availablePaperLinks" class="ks-paper-links-panel">
          <h3
          ...
          <ul class="ks-paper-links-panel__list">
            <li v-if="availablePaperLinks.code_url">
              <a
                :href="availablePaperLinks.code_url"
                target="_blank"
                rel="noopener"
                class="ks-paper-links-panel__link"
              >
                ...
                <span class="ks-paper-links-panel__label">{{
                  isZh ? "代码" : "Code"
                }}</span>
            ...
            <li v-if="availablePaperLinks.project_page_url">
              <a
                :href="availablePaperLinks.project_page_url"
                target="_blank"
                rel="noopener"
                class="ks-paper-links-panel__link"
              >
                <span class="ks-paper-links-panel__icon" aria-hidden="true">
                  ...
                  >
                    <path
                      d="M10.427 5.376c-.668-.31-1.379-.26-1.851.164L3.302 9.652a1.79 1.79 0 0 0-.087 2.565l5.894 6.269 1.403-1.484-5.16-5.48 4.683-3.813.045-.037c.019-.015.034-.023.04-.016.006.007 0 .024-.006.04l-.027.063L7.08 12.21l1.658.8 3.214-6.671c.276-.573.141-1.252-.303-1.764zm8.845 10.907-5.897-6.269-1.4 1.484 5.156 5.48-4.679 3.81-.045.037c-.018.015-.033.023-.04.016-.005-.007.001-.024.007-.04l.026-.063 3.011-4.447-1.658-.801-3.214 6.671c-.275.573-.141 1.252.303 1.764.667.31 1.378.26 1.85-.164l5.274-4.112a1.79 1.79 0 0 0 .087-2.566z"
                    />
                  </svg>
                </span>
                <span class="ks-paper-links-panel__label">{{
            ...
            <li v-if="availablePaperLinks.model_weights_url">
              <a
                :href="availablePaperLinks.model_weights_url"
                target="_blank"
                rel="noopener"
                class="ks-paper-links-panel__link"
              >
            ...
            <template v-if="availablePaperLinks.dataset_urls?.length">
              <li
                v-for="(url, i) in availablePaperLinks.dataset_urls ?? []"
                :key="`ds-${i}`"
              >
                <a
                  :href="url"
                ...
                >
                  ...
                  <span class="ks-paper-links-panel__label">{{
                    isZh
                      ? `数据集${(availablePaperLinks.dataset_urls?.length ?? 0) > 1 ? ` ${i + 1}` : ""}`
                      : `Dataset${(availablePaperLinks.dataset_urls?.length ?? 0) > 1 ? ` ${i + 1}` : ""}`
                  }}</span>
                  <KsTag variant="neutral" style="font-size: 0.625rem"
                    >dataset</KsTag
                  >
            ...
            <li v-if="availablePaperLinks.related_links?.blog_url">
              <a
                :href="availablePaperLinks.related_links.blog_url"
                target="_blank"
                rel="noopener"
                class="ks-paper-links-panel__link"
              >
            ...
            <li v-if="availablePaperLinks.related_links?.discussion_url">
              <a
                :href="availablePaperLinks.related_links.discussion_url"
                target="_blank"
                rel="noopener"
                class="ks-paper-links-panel__link"
              >
            ...
            <li v-if="availablePaperLinks.related_links?.social_url">
              <a
                :href="availablePaperLinks.related_links.social_url"
                target="_blank"
                rel="noopener"
                class="ks-paper-links-panel__link"
              >
        ...
        <div v-if="paperLabels" class="ks-paper-labels">
          ...
          <div
            ...
            :key="dim.key"
            class="ks-paper-labels__group"
          >
            <span class="ks-paper-labels__dim" :style="{ color: dim.color }">{{
              dim.label
            }}</span>
