...
const projectCover = computed<ProjectCoverProps>(() => ({
  ...
  description: collection.value?.description || 'Investigating atomic claim extraction and evidence alignment for biomedical retrieval-augmented generation.',
  paperCount: collection.value?.paper_count ?? mockPapers.length,
  memberCount: 1,
  lastUpdated: collection.value?.updated_at ? new Date(collection.value.updated_at).toLocaleDateString() : '2 hours ago',
  status: 'active',
}))
...
const corpusPapers = computed<CorpusPaper[]>(() =>
  papers.value?.length
    ? papers.value.map((paper, i) => ({
        ...
        authors: paper.raw_metadata?.author_names?.join(', ') || 'Unknown authors',
        year: paper.published_at ? new Date(paper.published_at).getFullYear() : new Date().getFullYear(),
        status: paper.reading_status === 'read'
          ? 'read'
    ...
    <div class="ks-workspace__content">
      ...
      <section class="ks-card ks-workspace__tasks">
        ...
        <ul v-else class="ks-workspace__task-list">
          <li v-for="task in tasks" :key="task.id" class="ks-workspace__task-item">
            <label class="ks-workspace__task-label">
              <input :checked="task.completed" type="checkbox" @change="handleTaskToggle(task)">
