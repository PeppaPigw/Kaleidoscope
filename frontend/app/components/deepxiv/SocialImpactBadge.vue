    ...
    <template v-if="impact">
      <div class="ks-social-impact__metrics">
        <div class="ks-social-impact__metric">
          ...
          <span class="ks-social-impact__value">{{ formatCount(impact.total_tweets) }}</span>
          <span class="ks-social-impact__label">tweets</span>
        </div>
        <div class="ks-social-impact__metric">
          <Heart :size="14" />
          <span class="ks-social-impact__value">{{ formatCount(impact.total_likes) }}</span>
          <span class="ks-social-impact__label">likes</span>
        </div>
        <div class="ks-social-impact__metric">
          <Eye :size="14" />
          <span class="ks-social-impact__value">{{ formatCount(impact.total_views) }}</span>
          <span class="ks-social-impact__label">views</span>
        </div>
        <div class="ks-social-impact__metric">
          <MessageSquare :size="14" />
          <span class="ks-social-impact__value">{{ formatCount(impact.total_replies) }}</span>
          <span class="ks-social-impact__label">replies</span>
        </div>
      </div>
      <div class="ks-social-impact__dates">
        {{ formatDate(impact.first_seen_date) }} &ndash; {{ formatDate(impact.last_seen_date) }}
      </div>
    </template>
    <div v-else class="ks-social-impact__empty">
