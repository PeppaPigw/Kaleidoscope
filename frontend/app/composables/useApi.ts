...
export interface PaperArxivResolution {
  paper_id: string;
  arxiv_id: string;
  ingestion_status: string;
}

export interface PaperArxivResolutionResponse {
  matches: PaperArxivResolution[];
}
...
function describeAlertCondition(condition: unknown): string {
  if (!condition || typeof condition !== "object") return "{}";

  const record = condition as {
    keywords?: unknown;
    author_names?: unknown;
    min_citations?: unknown;
    venue_ids?: unknown;
  };

  if (Array.isArray(record.keywords) && record.keywords.length > 0) {
    return `Keywords: ${record.keywords.join(", ")}`;
  }
  if (Array.isArray(record.author_names) && record.author_names.length > 0) {
    return `Authors: ${record.author_names.join(", ")}`;
  }
  if (Array.isArray(record.venue_ids) && record.venue_ids.length > 0) {
    return `Venues: ${record.venue_ids.join(", ")}`;
  }
  if (typeof record.min_citations === "number") {
    return `Minimum citations: ${record.min_citations}`;
  }
...
export interface AnalysisFieldCompletion {
  method: number;
  dataset: number;
  metric: number;
...
export interface AnalysisCoverage {
  ...
  main_result_records: number;
  methods: number;
  datasets: number;
  metrics: number;
  field_completion: AnalysisFieldCompletion;
  overall: number;
}
...
export interface AnalysisPaperInfo {
  id: string;
  title: string;
  year?: number | null;
  source: string;
...
export interface AnalysisExperimentRecord {
  id: string;
  paper_id: string;
  paper_title: string;
...
export interface AnalysisMatrixColumn {
  key: string;
  dataset: string;
  metric: string;
  higher_is_better: boolean;
...
export interface AnalysisMatrixRow {
  ...
  numeric_metrics: Record<string, number | null>;
  best_metrics: string[];
  is_best: boolean;
  record_ids: string[];
...
export interface AnalysisMatrix {
  columns: AnalysisMatrixColumn[];
  metric_names: string[];
  rows: AnalysisMatrixRow[];
  best_by_column: Record<string, string>;
}

export interface ExtractExperimentsResponse {
  paper_id: string;
  title: string;
  paper: AnalysisPaperInfo;
  experiments: Array<Record<string, unknown>>;
  methods: AnalysisMethodSummary[];
  records: AnalysisExperimentRecord[];
  matrix_ready: AnalysisMatrix;
  coverage: AnalysisCoverage;
  confidence: AnalysisConfidence;
}

export interface AnalysisMethodsFilters {
  datasets: string[];
  metrics: string[];
  main_results_only: boolean;
  include_uncomparable: boolean;
}

export interface AnalysisMethodsRequest {
  paper_ids?: string[];
  collection_id?: string;
  research_question?: string;
  filters?: Partial<AnalysisMethodsFilters>;
  refresh_missing?: boolean;
}

export interface AnalysisMethodsScope {
  type: string;
  paper_ids: string[];
  paper_count: number;
...
export interface AnalysisVectorHit {
  paper_id: string;
...
export interface AnalysisQASupport {
  status: string;
  queued: boolean;
  answer?: string | null;
...
export interface AnalysisMethodsPaper {
  id: string;
  title: string;
  year?: number | null;
  ingestion_status: string;
  ...
  confidence: AnalysisConfidence;
  record_count: number;
  qa_support?: AnalysisQASupport | null;
}

export interface AnalysisMethodsRowSummary {
  id: string;
  paper_id: string;
  paper_title: string;
  method_name: string;
...
export interface AnalysisCrossPaperSummary {
  answer?: string | null;
  sources: Array<Record<string, unknown>>;
  chunks_found: number;
  latency_ms?: number | null;
...
export interface AnalysisEvidence {
  scope_hits: AnalysisVectorHit[];
  paper_hits: Record<string, AnalysisVectorHit[]>;
}

export interface AnalysisContradictionCandidate {
  dataset: string;
  metric: string;
  methods: string[];
  reason: string;
...
export interface AnalysisMethodsCoverage {
  overall: number;
  papers_total: number;
  papers_with_records: number;
  papers_with_scope_hits: number;
  records_total: number;
  records_after_filters: number;
}

export interface AnalysisMethodsResponse {
  scope: AnalysisMethodsScope;
  research_question: string;
  papers: AnalysisMethodsPaper[];
  methods: AnalysisMethodsRowSummary[];
  datasets: string[];
  metrics: string[];
  matrix: AnalysisMatrix;
  filters_applied: AnalysisMethodsFilters;
  cross_paper_summary?: AnalysisCrossPaperSummary | null;
  evidence: AnalysisEvidence;
  contradiction_candidates: AnalysisContradictionCandidate[];
  coverage: AnalysisMethodsCoverage;
  warnings: string[];
}
...
export function useApi() {
  ...
  async function resolvePapersByArxivIds(
    arxivIds: string[],
  ): Promise<PaperArxivResolutionResponse> {
    return apiFetch("/papers/resolve-arxiv", {
      method: "POST",
      body: { arxiv_ids: arxivIds },
    });
  }
  ...
  async function getCollectionPapers(
    collectionId: string,
  ...
  async function listCollectionChildren(
    ...
    const qs = searchParams.toString();
    return apiFetch(
      `/collections/${collectionId}/children${qs ? `?${qs}` : ""}`,
    );
  }
  ...
  async function listCollectionThreadMessages(
    ...
    return apiFetch(
      `/collections/${collectionId}/threads/${threadId}/messages`,
    );
  }
  ...
  async function getWritingDocument(
    documentId: string,
  ...
  async function uploadWritingImage(
    file: File,
  ): Promise<WritingImageUploadResponse> {
  ...
  async function getComments(
    ...
    return {
      comments: Array.isArray(response) ? response : (response.comments ?? []),
    };
  }
  ...
  async function completeTask(taskId: string): Promise<void> {
    await apiFetch(`/collaboration/tasks/${taskId}/complete`, {
      method: "PATCH",
    });
  }
  ...
  async function createAlertRule(input: {
    ...
    const rule = await apiFetch<AlertRule & { condition: unknown }>(
      "/alerts/rules",
      {
        method: "POST",
        body: {
    ...
    return {
      ...
      condition:
        typeof rule.condition === "string"
          ? rule.condition
          : describeAlertCondition(rule.condition),
    };
  }
  ...
  async function getAlertRules(): Promise<{ rules: AlertRule[] }> {
    const response = await apiFetch<{
      rules: Array<AlertRule & { condition: unknown }>;
    }>("/alerts/rules");
    return {
  ...
  async function getAnalysisMethods(
    input: AnalysisMethodsRequest = {},
  ): Promise<AnalysisMethodsResponse> {
    const filters: AnalysisMethodsFilters = {
      datasets: input.filters?.datasets ?? [],
      metrics: input.filters?.metrics ?? [],
      main_results_only: input.filters?.main_results_only ?? false,
      include_uncomparable: input.filters?.include_uncomparable ?? false,
    };

    return apiFetch("/analysis/methods", {
      method: "POST",
      body: {
        ...
        filters,
        refresh_missing: input.refresh_missing ?? false,
      },
    });
  }
  ...
  async function extractExperiments(
    paperId: string,
  ): Promise<ExtractExperimentsResponse> {
    return apiFetch(`/analysis/papers/${paperId}/extract-experiments`, {
      method: "POST",
    });
  }
  ...
  async function getAnalyticsTopAuthors(
    limit = 20,
  ): Promise<AnalyticsTopAuthors> {
    return apiFetch(`/analytics/top-authors?limit=${limit}`);
  }

  async function getAnalyticsKeywordCloud(
    limit = 50,
  ): Promise<AnalyticsKeywordCloud> {
  ...
  async function buildOpenAlexGraph(
    paperIds: string[],
  ): Promise<OpenAlexGraphResponse> {
