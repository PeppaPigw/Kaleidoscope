  ...
  meta: [
    {
      name: "description",
      content:
        "Chat across your workspaces, subscription collections, and paper groups.",
    },
  ],
});
...
const collectionMap = computed(
  () => new Map(collections.value.map((c) => [c.id, c])),
);
const workspaces = computed(() =>
  collections.value.filter((c) => c.kind === "workspace"),
);
const subscriptionCollections = computed(() =>
  collections.value.filter((c) => c.kind === "subscription_collection"),
);
const paperGroups = computed(() =>
  collections.value.filter((c) => c.kind === "paper_group"),
);
const activeCollection = computed(() =>
  activeCollectionId.value
    ? (collectionMap.value.get(activeCollectionId.value) ?? null)
    : null,
);
const groupParentCollection = computed(() => {
  if (!activeCollection.value || activeCollection.value.kind !== "paper_group")
    return null;
...
const activeParentContainer = computed(() => {
  ...
  if (activeCollection.value.kind === "paper_group")
    return groupParentCollection.value;
...
const createGroupParent = computed(() => {
  if (!activeCollection.value)
    return workspaces.value[0] ?? subscriptionCollections.value[0] ?? null;
  if (activeCollection.value.kind === "paper_group")
    return groupParentCollection.value;
  if (
    activeCollection.value.kind === "workspace" ||
    activeCollection.value.kind === "subscription_collection"
  )
    return activeCollection.value;
  return null;
});
...
function kindMonoLabel(kind: Collection["kind"]): string {
  const normalized = normalizeCollectionKind(kind);
  if (normalized === "subscription_collection")
...
function sourceCount(message: CollectionChatMessage): number {
  return Array.isArray(message.sources?.items)
    ? (message.sources?.items?.length ?? 0)
    : 0;
}
...
async function loadCollections() {
  ...
  try {
    ...
    if (
      !activeCollectionId.value ||
      !collectionMap.value.has(activeCollectionId.value)
    ) {
      activeCollectionId.value =
        workspaces.value[0]?.id ??
        subscriptionCollections.value[0]?.id ??
        paperGroups.value[0]?.id ??
        null;
    }
  } catch (error: unknown) {
    pageError.value =
...
async function loadActiveCollectionContext() {
  ...
  try {
    ...
    if (collection.kind === "paper_group" && collection.parent_collection_id) {
      const parentPaperResponse = await api.getCollectionPapers(
    ...
    messages.value = threadId
      ? await api.listCollectionThreadMessages(collection.id, threadId)
      : [];
  } catch (error: unknown) {
    pageError.value =
...
async function createSubscriptionCollection() {
  ...
  const created = await api.createCollection({
    name,
    description: subscriptionDescription.value.trim() || undefined,
    kind: "subscription_collection",
  });
  if (selectedFeedIds.value.length)
    await api.attachCollectionFeeds(created.id, selectedFeedIds.value);
...
async function createPaperGroup() {
  ...
  const created = await api.createCollection({
    name,
    description: `Paper group inside ${parent.name}`,
    kind: "paper_group",
...
async function createFreshThread() {
  ...
  const created = await api.createCollectionThread(
...
async function reloadActiveMessages() {
  ...
  messages.value = await api.listCollectionThreadMessages(
    activeCollection.value.id,
    activeThreadId.value,
  );
}
...
async function syncActiveCollection() {
  ...
  syncing.value = true;
  try {
    await api.triggerCollectionSync(activeCollection.value.id);
  } catch (error: unknown) {
    pageError.value =
...
async function addSelectedParentPapersToGroup() {
  ...
  await api.addPapersToCollection(
    activeCollection.value.id,
    selectedParentPaperIds.value,
    "Added from landscape group manager",
  );
  selectedParentPaperIds.value = [];
  ...
  async (newMessages) => {
    for (const msg of newMessages) {
      if (
        msg.role === "assistant" &&
        msg.content &&
        !renderedMessages.value[msg.id]
      ) {
        try {
          renderedMessages.value[msg.id] = await renderRagflowAnswer(
            msg.content,
          );
        } catch {
          renderedMessages.value[msg.id] =
    ...
    <aside class="rc-left">
      ...
      <div class="rc-left__list">
        ...
        <div class="rc-section">
          ...
          <transition name="form-drop">
            <div v-if="showWorkspaceForm" class="rc-form">
              ...
              <textarea
                ...
                rows="2"
                class="rc-form__input rc-form__input--area"
                placeholder="What is this workspace for?"
              />
              <button
                type="button"
                class="rc-form__submit"
          ...
          >
            ...
            <div class="rc-coll-card__foot">
              <span class="rc-coll-card__meta"
                >{{ ws.paper_count ?? 0 }} papers</span
              >
              <svg
                width="14"
                height="14"
        ...
        <div class="rc-section">
          <div class="rc-section__head">
            ...
            <button
              type="button"
              class="rc-new-btn"
              @click="showSubscriptionForm = !showSubscriptionForm"
            >
          ...
          <transition name="form-drop">
            <div v-if="showSubscriptionForm" class="rc-form">
              ...
              <textarea
                ...
                rows="2"
                class="rc-form__input rc-form__input--area"
                placeholder="Describe this feed cluster…"
              />
              <div class="rc-feed-picker">
                <label
                  v-for="feed in feedCatalog.slice(0, 8)"
                  :key="feed.id"
                  class="rc-feed-opt"
                >
                  <input
              ...
              <button
                ...
                @click="createSubscriptionCollection"
              >
                Create subscription
              </button>
            </div>
          ...
          >
            <p class="rc-coll-card__kind rc-coll-card__kind--gold">
              SUBSCRIPTION
            </p>
            <h4 class="rc-coll-card__name">{{ sc.name }}</h4>
            <div class="rc-coll-card__foot">
              <span class="rc-coll-card__meta"
                >{{ sc.paper_count ?? 0 }} papers</span
              >
              <svg
                width="14"
                height="14"
        ...
        <div class="rc-section">
          ...
          <transition name="form-drop">
            <div v-if="showGroupForm" class="rc-form">
              ...
              <p class="rc-muted">
                Parent:
                <strong>{{
                  createGroupParent?.name || "Select a workspace first"
                }}</strong>
              </p>
              <button
                type="button"
                class="rc-form__submit"
                :disabled="!groupName.trim() || !createGroupParent"
                @click="createPaperGroup"
              >
                Create group
              </button>
            </div>
          ...
          >
            <p class="rc-coll-card__kind rc-coll-card__kind--ink">
              PAPER GROUP
            </p>
            <h4 class="rc-coll-card__name">{{ pg.name }}</h4>
            <div class="rc-coll-card__foot">
              <span class="rc-coll-card__meta">{{
                collectionMap.get(pg.parent_collection_id || "")?.name ||
                "Standalone"
              }}</span>
              <svg
                width="14"
      ...
      <template v-if="activeCollection">
        ...
        <header class="rc-center__header">
          <div>
            ...
            <p class="rc-center__subtitle">
              {{ kindMonoLabel(activeCollection.kind) }} ·
              {{ activeCollectionPapers.length }} papers indexed
            </p>
          </div>
          <div class="rc-center__actions">
            ...
            <div v-if="threads.length > 1" class="rc-thread-tabs">
              <button
                ...
                class="rc-thread-tab"
                :class="{
                  'rc-thread-tab--active': activeThreadId === thread.id,
                }"
        ...
        <div class="rc-messages">
          ...
          <div v-if="loadingContext" class="rc-state-center">
            <div class="rc-dots"><span /><span /><span /></div>
          </div>
              ...
              <div v-else class="rc-msg__ai-card">
                ...
                <div v-if="sourceCount(msg) > 0" class="rc-sources">
                  ...
                  >
                    ...
                    {{ sourceCount(msg) }} cited source{{
                      sourceCount(msg) > 1 ? "s" : ""
                    }}
                  </button>
                  <div v-if="expandedSources[msg.id]" class="rc-sources__list">
                    <div
                      v-for="(src, i) in msg.sources?.items ?? []"
                      :key="i"
                      class="rc-source-item"
                    >
                      <span class="rc-source-item__idx">[{{ i + 1 }}]</span>
                      <div>
                        <p
                          v-if="src.section_title || src.title"
                          class="rc-source-item__title"
                        >
                          {{ src.section_title || src.title }}
                        </p>
                        <p
                          v-if="src.text_snippet || src.text || src.content"
                          class="rc-source-item__snippet"
                        >
        ...
        <div class="rc-composer">
          <form class="rc-composer__form" @submit.prevent="sendMessage">
            <div
              class="rc-composer__inner"
              :class="{ 'rc-composer__inner--focused': inputFocused }"
            >
              <input
                v-model="chatInput"
                type="text"
              ...
              <button
                ...
                :class="{
                  'rc-composer__send--active': chatInput.trim() && !sending,
                }"
          ...
          <p class="rc-composer__hint">
            Press Enter to send ·
            {{ activeCollection.paper_count ?? activeCollectionPapers.length }}
            papers available
          </p>
        </div>
      </template>
      ...
      <template v-if="activeCollection">
        ...
        <section class="rc-right__hero">
          <div class="rc-eyebrow-row">
            ...
            <p class="rc-eyebrow rc-eyebrow--teal">
              {{ kindLabel(activeCollection.kind) }}
            </p>
          </div>
        ...
        <section class="rc-stats-grid">
          <div class="rc-stats-grid__cell">
            ...
            <p class="rc-stats-grid__value">
              {{ activeCollectionPapers.length }}
            </p>
          </div>
          <div class="rc-stats-grid__cell">
          ...
          <div class="rc-stats-grid__cell rc-stats-grid__cell--last">
            ...
            <p class="rc-stats-grid__value rc-stats-grid__value--gold">
              {{ kindLabel(activeCollection.kind) }}
            </p>
          </div>
        </section>
        ...
        <!-- Feeds (subscription) -->
        <section
          v-if="activeCollection.kind === 'subscription_collection'"
          class="rc-right__section"
        >
          <p class="rc-eyebrow">Attached Feeds</p>
          <ul class="rc-right__list">
            <li
              v-for="feed in collectionFeeds"
              :key="feed.id"
              class="rc-right__list-item"
            >
              <strong>{{ feed.feed_name || feed.feed_id }}</strong>
              <small v-if="feed.publisher">{{ feed.publisher }}</small>
            </li>
            <li v-if="!collectionFeeds.length" class="rc-muted">
              No feeds attached yet.
            </li>
          </ul>
        </section>
        ...
        <section
          v-if="activeCollection.kind === 'paper_group'"
          class="rc-right__section"
        >
          ...
          <div class="rc-paper-picker">
            <label
              v-for="paper in parentCollectionPapers.slice(0, 8)"
              :key="paper.id"
              class="rc-paper-opt"
            >
              <input
        ...
        <section class="rc-right__section">
          ...
          <ul class="rc-right__list">
            <li
              v-for="paper in activeCollectionPapers.slice(0, 10)"
              :key="paper.id"
              class="rc-right__list-item"
            >
            ...
            <li v-if="!activeCollectionPapers.length" class="rc-muted">
              No papers yet.
            </li>
          </ul>
          <p
            v-if="activeCollectionPapers.length > 10"
            class="rc-muted"
...
.rc-dots span:nth-child(2) {
  animation-delay: 0.2s;
}
.rc-dots span:nth-child(3) {
...
.rc-coll-card__kind--gold {
  color: var(--rc-tertiary);
}
.rc-coll-card__kind--ink {
  color: #3949ab;
}
...
.rc-msg__body :deep(ol:first-child) {
  margin-top: 0;
}

.rc-msg__body :deep(p:last-child),
.rc-msg__body :deep(ul:last-child),
.rc-msg__body :deep(ol:last-child) {
  margin-bottom: 0;
}

.rc-msg__body :deep(p),
.rc-msg__body :deep(li) {
  color: var(--rc-on-surface-variant);
}
