...
describe("useApi.collections", () => {
  ...
  it("sends collection kind and parent filters when listing typed collections", async () => {
    ...
    expect(requestUrl.searchParams.get("parent_collection_id")).toBe(
      "workspace-1",
    );
  });
  ...
  it("creates typed collections and calls the new thread/feed endpoints", async () => {
    ...
    expect(new URL(fetchMock.mock.calls[1]?.[0] as string).pathname).toBe(
      "/api/v1/collections/collection-9/feeds",
    );
    expect(new URL(fetchMock.mock.calls[2]?.[0] as string).pathname).toBe(
      "/api/v1/collections/collection-9/threads",
    );
    expect(new URL(fetchMock.mock.calls[3]?.[0] as string).pathname).toBe(
      "/api/v1/collections/collection-9/threads/thread-1/ask",
    );
    ...
    expect(new URL(fetchMock.mock.calls[4]?.[0] as string).pathname).toBe(
      "/api/v1/collections/collection-9/papers",
    );
  ...
  it("fetches collection detail, collection papers, and researcher profile endpoints", async () => {
    ...
    expect(new URL(fetchMock.mock.calls[0]?.[0] as string).pathname).toBe(
      "/api/v1/collections/collection-3",
    );
    expect(new URL(fetchMock.mock.calls[1]?.[0] as string).pathname).toBe(
      "/api/v1/collections/collection-3/papers",
    );
    expect(new URL(fetchMock.mock.calls[2]?.[0] as string).pathname).toBe(
      "/api/v1/researchers/author-1/profile",
    );
...
describe("useApi.writing", () => {
  ...
  it("lists, creates, updates, and deletes writing documents through the writing endpoints", async () => {
    ...
    expect(new URL(fetchMock.mock.calls[2]?.[0] as string).pathname).toBe(
      "/api/v1/writing/documents/doc-1",
    );
