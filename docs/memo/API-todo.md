...
- Current running OpenAPI operations verified: 308, plus explicit `/health` replay
...
- [x] Verify every public endpoint has auth/scope/rate-limit behavior suitable for external callers — `/api/v1/*`, `/api/openapi.json`, `/health`, and `/health/services` now require an API key; the default development key is `sk-kaleidoscope`; generated `ks_live_*` API keys remain supported through `Authorization: Bearer`; JWT remains supplementary for user identity after the API key gate; key scopes are attached to request state/headers, and deployment rate-limit policy metadata is emitted.
...
- [x] Direct curl-check the API key gate — missing key returns `401 API_KEY_REQUIRED`, invalid key returns `401 INVALID_API_KEY`, and `X-API-Key: sk-kaleidoscope` returns `200` for `/api/v1/auth/me` and `/health`.
- [x] Direct HTTP smoke-test every current running OpenAPI operation — `backend/app/scripts/verify_all_api_runtime.py --base-url http://127.0.0.1:8000 --api-key sk-kaleidoscope` sent 309 real HTTP requests (`308` OpenAPI operations plus explicit `/health` replay) against the running dev backend and generated `docs/memo/full-api-runtime-report.md` plus `docs/memo/full-api-runtime-report.json` with `309/309 passed`.
