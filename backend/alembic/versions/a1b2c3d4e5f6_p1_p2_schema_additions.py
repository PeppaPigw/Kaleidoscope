...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "filter_query", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        op.f("ix_user_reading_statuses_user_id"),
        "user_reading_statuses",
        ["user_id"],
        unique=False,
    )
    ...
    op.add_column(
        "papers",
        sa.Column(
            "source_type", sa.String(length=20), nullable=False, server_default="remote"
        ),
    )
    op.add_column(
        "papers",
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_user_reading_statuses_user_id"), table_name="user_reading_statuses"
    )
