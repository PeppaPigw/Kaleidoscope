...
export interface SemanticHighlight {
  id: string
  text: string
  category: 'hypothesis' | 'methodology' | 'result' | 'limitation' | 'contribution'
  confidence: number
  page: number
}
...
<template>
  <section class="ks-semantic-highlights ks-motion-paper-reveal" :aria-labelledby="`${uid}-title`">
    ...
    <div class="ks-semantic-highlights__list">
      ...
      >
        <div class="ks-semantic-highlights__header">
          ...
          <span class="ks-type-data" style="color: var(--color-primary);">{{ (h.confidence * 100).toFixed(0) }}%</span>
        </div>
